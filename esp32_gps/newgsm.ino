...
void loop()
{
  ...
  while (true)
  {
    ...
    sed = String(gps.speed.kmph(), 6);

    root["LA"] = lat;
    root["LO"] = lo;
    root["TIME"] = hours + mi + se;
    root["DATE"] = days + yeara + months;
    root["SP"] = sed;
    ...
    else
    {
...

      {
        ...
        gpsData += "\"Date\":" + days + yeara + months + ",";
        gpsData += "\"SP\":" + sed + "";
        gpsData += "}";
        putToFirebase("/", "/GPS4/", jsonStr, &http_client);
...
void ADXL345()
{
  ...
  if (a > 1.2)
  {
    ...
    gpsData += "\"Speed\":" + sed + "";
    gpsData += "}";
    postToFirebase("/", "/GPS2/", jsonStr, &http_client);
    //
  }
...
void vitri()
{
  if (gps.location.isValid())
  {
    ...
    gpsData += "\"Speed\":" + sed + "";
    gpsData += "}";
    postToFirebase("/", "/GPS3/", jsonStr, &http_client);
  }
}
